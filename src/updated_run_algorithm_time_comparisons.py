...
def get_total_latency_ms(user_pos, g=100, t_seconds=0.0, pool_llm_max_time=None, new_L=None, new_alpha=None, ranker_time=RANKER_COMPUTE_SEC, fuser_time=FUSER_COMPUTE_SEC):
    ...
    # return route
    route["time"] = time.time() - start_time
    return route
...
def get_random_total_latency_ms(user_pos, g=100, t_seconds=0.0, pool_llm_max_time=None, new_L=None, new_alpha=None, ranker_time=RANKER_COMPUTE_SEC, fuser_time=FUSER_COMPUTE_SEC):
    ...
    if route is None:
        return INVALID_TOTAL_LATENCY_MS
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        SS_time += get_total_latency_ms(user_pos, g=1000, pool_llm_max_time=3, new_L=3, new_alpha=27)["time"]
        GSS_time += get_total_latency_ms_fancy(user_pos, g=1000, pool_llm_max_time=3, new_L=3, new_alpha=27)["time"]

    print(f"SS_time: {SS_time}")
    print(f"GSS_time: {GSS_time}")
