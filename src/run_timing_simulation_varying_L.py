...
for i in range(simulations_per_datapoint):
    ...
    time_exp = get_total_latency_ms(user_pos, pool_llm_max_time=args.pool_llm_time_limit, g=args.g, new_L=int(args.L), new_alpha=args.alpha, ranker_time=ranking_time, fuser_time=fuser_time) # this is for one prompt
    random_time = get_random_total_latency_ms(user_pos, pool_llm_max_time=args.pool_llm_time_limit, g=args.g, new_L=int(args.L), new_alpha=args.alpha, ranker_time=ranking_time, fuser_time=fuser_time)
    closest_time = get_total_latency_ms_closest(user_pos, pool_llm_max_time=args.pool_llm_time_limit, g=args.g, new_L=int(args.L), new_alpha=args.alpha, ranker_time=ranking_time, fuser_time=fuser_time)

    with open(f"final_timing_data_varying_L_{args.L}.csv", "a", newline="\n") as file:
        writer = csv.writer(file)
        writer.writerow([args.L, time_exp, random_time, closest_time])