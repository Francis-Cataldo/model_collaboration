...
def _summarize_latency(latency_logs):
    total = [x["total_pipeline_sec"] for x in latency_logs]
    gen = [x["candidate_generation_sec"] for x in latency_logs]
    rank = [x["ranking_sec"] for x in latency_logs]
    fuse = [x["fusion_sec"] for x in latency_logs]

    def summary_one(values):
        if not values:
            return {}
        return {
    ...
    return {
        "candidate_generation_sec": summary_one(gen),
        "ranking_sec": summary_one(rank),
        "fusion_sec": summary_one(fuse),
        "total_pipeline_sec": summary_one(total),
    }
...
def _run_method_latency_distribution(
    task,
    task_type,
    gpu_ids,
    model_names,
    ...
    """
    Latency-distribution version of text_llm_blender.

    Runs one test example at a time so that we can estimate
    per-example delay distribution:
    ...
    max_examples = hyperparameters.get("latency_max_examples")
    if max_examples is not None:
        test_input_list = test_input_list[: int(max_examples)]
    ...
    max_response_length = hyperparameters.get("max_response_length")
    top_k = hyperparameters.get("top_k", 3)
    top_k = max(1, min(top_k, num_models))

    base_ranker = hyperparameters.get(
        "ranker_model_name", "Qwen/Qwen2.5-7B-Instruct"
    )
    actual_ranker_model = base_ranker
    ranker_gpu_id = hyperparameters.get("ranker_gpu_id", gpu_ids[0])
    ranker_max_response_length = hyperparameters.get(
        "ranker_max_response_length", 128
    )
    ...
    actual_fuser_model = base_fuser
    fuser_gpu_id = hyperparameters.get("fuser_gpu_id", gpu_ids[0])
    fuser_max_response_length = hyperparameters.get(
        "fuser_max_response_length", max_response_length
    )

    print("[LLM-Blender-Latency] Regular LLM models:", model_names)
    print("[LLM-Blender-Latency] Ranker model:", actual_ranker_model)
    print("[LLM-Blender-Latency] Fuser model:", actual_fuser_model)
    print("[LLM-Blender-Latency] Number of test examples:", num_examples)

    final_outputs = []
    latency_logs = []
    detailed_logs = []

    for example_idx, question in enumerate(test_input_list):
        print(f"[LLM-Blender-Latency] Example {example_idx + 1}/{num_examples}")

        example_total_start = time.perf_counter()
        ...
        one_input_per_model = [[question] for _ in model_names]
        ...
        candidate_generation_sec = gen_end - gen_start

        candidate_answers = [
            list_of_output_list[model_idx][0]
        ...
        scores = [0.0 for _ in range(num_models)]
        ...
        if pairwise_prompts:
            ...
            for output_text, (i, j) in zip(ranker_outputs, pairwise_metadata):
                pref = _parse_pairwise_preference(output_text)
                if pref == "A":
                    scores[i] += 1.0
                elif pref == "B":
        ...
        rank_end = time.perf_counter()
        ranking_sec = rank_end - rank_start
        ...
        top_candidates = [
            (model_idx, candidate_answers[model_idx])
            for model_idx in top_indices
        ]
        ...
        final_answer = fuser_outputs_nested[0][0]
        final_outputs.append(final_answer)

        fuse_end = time.perf_counter()
        fusion_sec = fuse_end - fuse_start

        example_total_end = time.perf_counter()
        total_pipeline_sec = example_total_end - example_total_start

        latency_entry = {
            "example_idx": example_idx,
            "num_regular_llms_L": num_models,
            "num_total_roles_L_plus_2": num_models + 2,
        ...
        latency_logs.append(latency_entry)

        detailed_logs.append({
            "input": question,
            "candidate_answers": candidate_answers,
            "ranker_scores": scores,
            "top_k_model_indices": top_indices,
            "output": final_answer,
        ...
        print(
            "[LATENCY_PER_EXAMPLE] "
            f"idx={example_idx} "
            f"gen={candidate_generation_sec:.3f}s "
            f"rank={ranking_sec:.3f}s "
            f"fuse={fusion_sec:.3f}s "
            f"total={total_pipeline_sec:.3f}s"
        )
    ...
    latency_summary = _summarize_latency(latency_logs)

    experiment_logs = {
        "task": task,
        "task_type": task_type,
        "method": "text_llm_blender_latency",
        "model_names": model_names,
        "ranker_model_name": actual_ranker_model,
        "fuser_model_name": actual_fuser_model,
    ...
    log_filename = (
        f"model_collaboration/logs/"
        f"{task}_{len(model_names)}_llm_blender_latency.json"
    )

    with open(log_filename, "w") as f:
        json.dump(experiment_logs, f, indent=4)

    print("[LLM-Blender-Latency] Final score:", avg_test_score)
    print("[LLM-Blender-Latency] Latency summary:")
    print(json.dumps(latency_summary, indent=4))
    print("[LLM-Blender-Latency] Saved log to:", log_filename)
...
def run_method(task, task_type, gpu_ids, model_names, hyperparameters):
    ...
    """
    if hyperparameters.get("measure_latency_distribution", False):
        return _run_method_latency_distribution(
            task,
            task_type,
            gpu_ids,
            model_names,
